...
function createApp() {
  ...
  app.get('/genres', function(request, response) {
    getGenres()
      .then(function(api) {
        ...
        response.status(500).send('Unable to load genres!');
      });
  });
  ...
  */
  ...
  app.post('/guess', function(request, response, next) {
    ...
    if (!select.title || select.title.length == 0) {
      response.status(500).send('Titles are somehow missing!')
      return;
    }

    correctAnswer = select.title[select.index];
